...
// Chapter 8.
// Exercise 21 (08.26) Knight's Tour: Closed-Tour Test.
...
class Knight
{
    ...
    // Numbers of row and column of the first and last positions of a knight.
    public int FirstPositionRow { get; set; }
    public int FirstPositionColumn { get; set; }
    public int LastPositionRow { get; set; }
    public int LastPositionColumn { get; set; }

    /// <summary>
    /// Returns "true" if the first point is reachable from the last.
    /// </summary>
    /// <returns></returns>
    public bool TourIsClosed()
    {
        bool tourIsClosed = false;

        // Check all possible moves for the last position.
        for (int moveNumber = 0; moveNumber < horizontalMoves.Length; ++moveNumber)
        {
            // If a knight can reach the first position using one of possible moves, return true and break the loop.
            if (FirstPositionRow == (LastPositionRow + horizontalMoves[moveNumber])
                && FirstPositionColumn == (LastPositionColumn + verticalMoves[moveNumber]))
            {
                tourIsClosed = true;
                break;
            }
...
→ → ↑        ↑     ↑       ↑ ← ←     4           ↓     ↓       → → 7
    ...
    public void MakeMove(int moveNumber = 0)
    {
        ...
        // Mark new point as the las one.
        LastPositionRow = CurrentPositionRow;
        LastPositionColumn = CurrentPositionColumn;
    ...
    public void PrintResultsOnePosition()
    {
        ...
        if (NumberOfMovesMade == 63)
        {
            if (TourIsClosed())
            {
                Console.WriteLine("Congratiolations! You made a full closed tour!");
            }
            else
            {
    ...
    public void PrintResultsAllPositions(int successfulTours, int unsuccessfulTours, int closedTours)
    {
        Console.WriteLine("Game is over. 64 tour attempts made from every start position.");
        Console.WriteLine($"Successful tours: {successfulTours} ({closedTours} tours are closed)");
