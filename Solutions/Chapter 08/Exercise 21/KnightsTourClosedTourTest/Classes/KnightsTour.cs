...
class KnightsTour
{
    static void Main()
    {
        ...
        if (!autoMode || chooseStartPosition)
        {
            ...
            // Set current poisition's row and column to values entered by a user and also set it as knight's first position.
            kenobi.CurrentPositionRow = rowToSet;
            kenobi.CurrentPositionColumn = columnToSet;
            kenobi.FirstPositionRow = rowToSet;
            kenobi.FirstPositionColumn = columnToSet;
        ...
        if (autoMode)
        {
            ...
            if (!chooseStartPosition)
            {
                ...
                // After all moves are made, the console screen is cleared and game statistics is printed.
                Console.Clear();
                kenobi.PrintResultsAllPositions(successfulTours, unseccessfulTours, closedTours);
                Console.Write("Press any key to exit.");
                Console.ReadKey();
            }
        }
